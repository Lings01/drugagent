...
# ---------------------------------------------------------------- R18: stage-relative path args

def test_tool_call_reroots_relative_input_paths(tmp_path):
    """R18: the LLM passes stage-relative input paths ('01_target/x.pdb');
    Tool.call must re-root them against the project dir when they exist
    there and not in the CWD (run --root . from any folder)."""
    from drugagent.agent.loop import Tool
    proj = tmp_path / "proj"
    (proj / "01_target").mkdir(parents=True)
    (proj / "01_target" / "receptor.pdb").write_text("ATOM\nEND\n")
    ctx = _mk_ctx(tmp_path)
    seen = {}

    def fn(c, pdb: str, out: str | None = None, **kw):
        seen["pdb"] = pdb
        return {"ok": True, "pdb": pdb}

    t = Tool(name="t", description="t", parameters={"type": "object"}, fn=fn)
    import os
    cwd = os.getcwd()
    os.chdir(tmp_path)  # CWD without 01_target/
    try:
        res = t.call(ctx, pdb="01_target/receptor.pdb")
    finally:
        os.chdir(cwd)
    assert res["ok"]
    assert seen["pdb"].startswith(str(proj)), seen["pdb"]


def test_tool_call_leaves_nonpath_strings_alone(tmp_path):
    """SMILES with slashes / resnames must not be re-rooted."""
    from drugagent.agent.loop import Tool
    ctx = _mk_ctx(tmp_path)
    seen = {}

    def fn(c, smiles: str, resname: str, **kw):
        seen["smiles"], seen["resname"] = smiles, resname
        return {"ok": True}

    t = Tool(name="t2", description="t", parameters={"type": "object"}, fn=fn)
    t.call(ctx, smiles="CC/C=C/C(=O)O", resname="HET_A")
    assert seen["smiles"] == "CC/C=C/C(=O)O"
    assert seen["resname"] == "HET_A"
